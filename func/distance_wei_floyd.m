...
% Caio Seguin 28/06/2017
% Altered the case in which the user passes a connection length matrix.
% Originally, entries in D (when no transform is specified) with value 0
% where treated as absent connections. Now, D must have Inf in the case of
% no connections between two nodes.

% DISTANCE_WEI_FLOYD        Distance matrix (Floyd-Warshall algorithm)
%
%   [SPL,hops,Pmat] = distance_wei_floyd(D,transform)
%
%   Computes the topolog10ical length of the shortest possible path
...
if exist('transform','var') && ~isempty(transform)
    ...
    switch transform
        ...
        case 'log10'
            ...
            % if any((D<0) & D>1)
            if sum(sum(D<0 | D>=1)) > 0
                error('connection-strengths must be in the interval [0,1) to use the transform -log10(w_ij) \n')
            else
                SPL = -log10(D);
            end
        ...
        otherwise
            ...
            error('Unexpected transform type. Only "log10" and "inv" are accepted \n')
    end
